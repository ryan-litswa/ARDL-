...
# Check for Multicollinearity VIF < 5 for little to no Multicollinearity 
# Install and load the car package to calculate VIF
...
# IPS Test for Unemployment
ips_unemployment <- purtest(pdata$Unemployment_Rate, test = "ips", exo = "intercept")
summary(ips_unemployment)

# JUST IF P-value not <0.05
...
#IF GDP p-value not <0.05
diff_Inflation <- diff(pdata$Inflation)
ips_Inflation_diff <- purtest(diff_Inflation, test = "ips", exo = "intercept")
summary(ips_Inflation_diff)



#Step 2
#Lag-length selection 
# Lag length selection for panel data
lag_selection <- VARselect(pdata[, c("Unemployment_Rate", "GDP", "FDI", "Inflation", "Population_Rate")], lag.max = 5, type = "const")
lag_selection$selection  # Display optimal lag lengths based on AIC, BIC, HQ
...
# Loop over each country and apply dynlm for each one
...
#Graphing the OKun's Law for each country: 
